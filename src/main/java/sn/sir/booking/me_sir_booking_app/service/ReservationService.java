...
import sn.sir.booking.me_sir_booking_app.model.ReservationDTO;
...
import sn.sir.booking.me_sir_booking_app.util.WebUtils;
...
public class ReservationService {
    ...
    public String getReferencedWarning(final Long id) {
        final Reservation reservation = reservationRepository.findById(id)
                .orElseThrow(NotFoundException::new);
        final Customer reservationCustomer = customerRepository.findFirstByReservation(reservation);
        if (reservationCustomer != null) {
            return WebUtils.getMessage("reservation.customer.reservation.referenced", reservationCustomer.getId());
        }
        final Room reservationRoom = roomRepository.findFirstByReservation(reservation);
        if (reservationRoom != null) {
            return WebUtils.getMessage("reservation.room.reservation.referenced", reservationRoom.getId());
        }
        return null;
    }
